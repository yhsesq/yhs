...
void get_point_list_of_polygon(XPoint *points, int *numpoints)
{
	...
	*numpoints = trunc(radius*2);
	for (i=0; i<(*numpoints); i++)
	{
	    rad = (2*PI*i)/(*numpoints);
	    points[i].x = trunc(cos(rad)*radius) + xc;
	    points[i].y = trunc(sin(rad)*radius) + yc;
	}
	break;
       	...
       	for (i=0; i<160; i++)
	{
	    ...
	    points[i].x = trunc(cos(rad)*radius*0.5) + xc;
	    points[i].y = trunc(sin(rad)*radius*0.5) + yc;
	}
	*numpoints = 160;
	break;
    }
