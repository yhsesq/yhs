...
public class rfbProto {
  ...
  static final int FramebufferUpdate = 0, SetColourMapEntries = 1, Bell = 2,
    ServerCutText = 3;

  static final int SetPixelFormat = 0, FixColourMapEntries = 1, SetEncodings = 2,
  ...
  public rfbProto(String sshHost, int sshPort, String sshUser, String sshPasswd,
	   String vncHost, int vncPort, MindVNC v1) throws IOException {
    v = v1;

    this.vncHost = vncHost;
    this.vncPort = vncPort;

    fact = new SSHSocketFactory(sshHost, sshPort, new SSHPasswordAuthenticator(sshUser, sshPasswd));

    sock = fact.createSocket(vncHost, vncPort);
  ...
  public boolean connectAndAuthenticate(authenticationPanel authenticator) throws IOException {
	...
	String pw = authenticator.vncPassword.getText();
	...
	case VncAuthFailed:
	  ...
	  authenticator.retry("VNC access denied");
	  break;
	case VncAuthTooMany:
  ...
  public void doProtocolInitialisation(int[] encodings, int nEncodings) throws IOException {
