 ...
 * $Name:  $
 *****************************************************************************/
// (C) 2004 Yohann Sulaiman. Removed home dir kludge. Specify if required.
...
public class MindTerm extends Applet implements Runnable {
  ...
  static Frame			frame;
  static TerminalWin          	term;
  static SSHInteractiveClient 	client;
  static SSHInteractiveClient 	sshClone;
  static SSHStdIO             	console;
  ...
  public static MindTerm controller;
  ...
  public static void main(String[] argv) {
    controller    = new MindTerm(paramSSHProps, paramTermProps);
    ...
    try {
    (new Thread(SSH.getThreadGroup(), controller)).start(); // YS
    } catch (Exception e) {
...
//	  clientThread = new Thread(client);
	if(SSH.NETSCAPE_SECURITY_MODEL) {
	  ...
	  clientThread.start();
//	while(clientThread.isAlive()){Thread.currentThread().sleep(1000);}
	  clientThread.join(); // waits for thread
